...
height_uav = [height_max, height_max - gap_height, height_max - 2 * gap_height, height_max - 3 * gap_height]; % The flight height of each UAV.
...
for cluster_index = 1:num_uav
    ...
    elseif cluster_index == 3
        plot(x_start, y_start,'k',LineWidth=1.5);
    else
        plot(x_start, y_start,'y',LineWidth=1.5);
    end
end
...
for index_uav = 1:num_uav
    ...
    elseif index_uav == 3
        ...
        plot(x,k(index_uav) * x + bias(index_uav),'-.y',LineWidth=1.5);
        scatter(final_point(index_uav,1),final_point(index_uav,2),'y','filled');
        hold on;
        start_uav_init(index_uav,:) = [center_init_uav_mgu(index_uav,1) - tra_radius_init(index_uav),center_init_uav_mgu(index_uav,2)];
        scatter(center_init_uav_mgu(index_uav,1) - tra_radius_init(index_uav),center_init_uav_mgu(index_uav,2),'s','y','filled');
    end
    hold on;
end
...
trajectory_uav4_init = zeros(time_all_init(4),2);
trajectory_uav4_init(1,:) = start_uav_init(4,:);
...
for index_slot = 2:time_all_init(4)
    trajectory_uav4_init(index_slot,:) = calculateTargetPointOnCircle(center_init_uav_mgu(4,1), center_init_uav_mgu(4,2), tra_radius_init(4), trajectory_uav4_init(index_slot-1,1), trajectory_uav4_init(index_slot-1,2), theta_increase(4));
end
...
hold on;
scatter(trajectory_uav4_init(:,1),trajectory_uav4_init(:,2));
hold on;
for cluster_index = 1:num_uav
    x_start = center_init_uav_mgu(cluster_index, 1) + tra_radius_init(cluster_index) * cos(theta);
    y_start = center_init_uav_mgu(cluster_index, 2) + tra_radius_init(cluster_index) * sin(theta);
...
hold on;
scatter(final_point(4,1),final_point(4,2),'b','filled');
hold on;
scatter(center_init_uav_mgu(4,1) - tra_radius_init(4),center_init_uav_mgu(4,2),'s','r','filled');
...
interf_id = findFarthestPoints(center_final_uav_mgu, num_uav);
...
for index_slot = 1:num_slot
    ...
    pos_mgu_uav4 = pos_mgu_rl(res_cluster_init == 4,:);
    ...
    for index_uav = 1:num_uav_rl
        ...
        elseif uav_set_rl(index_uav) == 3
            ...
            pos_uav_rl(index_uav, :) = trajectory_uav4_init(index_slot,:);
            dis_uav4_mgu = sqrt(sum((pos_uav_rl(index_uav, :) - pos_mgu_uav4) .^ 2, 2) + height_uav(index_uav) ^ 2);
            temp4 = sort(dis_uav4_mgu);
            dis_list_serevr_uav4 = temp4(1:num_subc_uav);
            [~,server_mgu_id_uav4] = ismember(dis_uav4_mgu, dis_list_serevr_uav4);
            pos_mgu_server_uav4 = pos_mgu_uav4(server_mgu_id_uav4 ~= 0, :);
            pos_mgu_server_rl_entry(index_uav,:,:) = pos_mgu_server_uav4;
            % gain(index_uav,:) = beta_0 ./ sqrt(sum((pos_uav_rl(index_uav, :) - pos_mgu_server_uav3) .^ 2, 2) + height_uav(index_uav) ^ 2);
            dis_server = reshape(dis_uav4_mgu(server_mgu_id_uav4 ~= 0),1,[]);
            f_ag = C1 + C2 ./ (1 + exp(-(B1 + B2 .* (height_uav(index_uav) ./ dis_server))));
            gain(index_uav,:) = beta_0 .* f_ag .* dis_server .^ (-a);
        end
    end
